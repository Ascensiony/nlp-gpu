...
def create_model(opts, dl, logger, state_dict=None):
    ...
    if state_dict is not None and "model" in state_dict:
        params = {k.replace('module.', ''): v for k,
...
def create_data_loader(opts, split, logger, called_by_validator=False):
    ...
    dataset = eval(dataset_opts.proto)(
        split=split, ckpt_dir=opts.ckpt_dir, **dataset_opts)
...
class CheckpointSaver(object):
    def __init__(self, ckpt_dir, logger, seed, ckpt=None):
        ...
        if ckpt is not None:
            self.current_tag, self.current_epoch = self.extract_tag_and_step(
                ckpt)
    ...
    def save(self, state_dict, tag, current_epoch):
        ...
        tag = np.round(tag, 6)
        path = os.path.join(self.ckpt_dir, '{}_{}_{}.pth'.format(
            tag, current_epoch, self.seed))
...
class TrainingScheduler(object):
    ...
    def __repr__(self):
        ...
        s += str(json.dumps(dict(self.lr_decay_params),
                            indent=4, sort_keys=True)) + '\n'
        s += 'Early stopping' + "\n"
        s += '    {0}: {1}\n'.format("early_stop_limit", self.early_stop_limit)
        s += '    {0}: {1}\n'.format("metric_comp_func", self.metric_comp_func)
        s += '    {0}: {1}\n'.format("mode", self.mode)
        s += '    {0}: {1}\n'.format("current_best_metric",
                                     self.current_best_metric)
        s += ')'
        return s

    def state_dict(self):
        training_sched = {key: value for key,
                          value in self.__dict__.items() if key != 'scheduler'}
